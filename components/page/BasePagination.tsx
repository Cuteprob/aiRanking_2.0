...
function LinkTag({
  ...
  return (
    <Link
      ...
      title={title}
      className={cn(
        'flex-center size-8 rounded-[4px] border border-transparent bg-transparent text-[#6219F0] hover:cursor-pointer hover:bg-[#6219F0]/50',
        className,
      )}
    >
      {children}
  ...
  if (type === 'page') {
    return (
      <LinkTag
        href={route}
        title={page.toString()}
        className={page === currentPage ? 'border-white bg-[#6219F0] text-white' : ''}
      >
        {page}
      </LinkTag>
    );
